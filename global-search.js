...
(function(){
  ...
  function loadScript(id,src){return new Promise(resolve=>{const old=document.getElementById(id);if(old)old.remove();const s=document.createElement('script');s.id=id;s.src=src;s.async=false;s.onload=resolve;s.onerror=resolve;document.body.appendChild(s);});}
  ...
  (async()=>{
    // El buscador instantáneo se carga primero y trabaja solo con infracciones.
    await loadScript('centinelaInstantSearchScript','./buscador-instantaneo.js?v=20260916d');
    ...
    await loadScript('centinelaDefinitiveSearchScript','./buscador-definitivo.js?v=20260916d');
    await loadScript('centinelaSearchFixScript','./buscador-fix.js?v=20260916c');
  })();
