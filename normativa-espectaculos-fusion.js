...
(function(){
  ...
  const text=a=>String(a?.descripcion??a?.conducta??a?.texto??a?.contenido??a?.resumen??'');
  function lawFor(record){
    const n=norm(record?.normativa||'');
    if(n.includes('13/1999')||n.includes('espectaculos publicos')) return 'Ley 13/1999 de Espectáculos Públicos y Actividades Recreativas de Andalucía';
    if(n.includes('155/2018')) return 'Decreto 155/2018 de Andalucía';
    return null;
  }
  async function run(){
    ...
    for(const r of records){const name=lawFor(r);if(!name)continue;(groups.get(name)||groups.set(name,[]).get(name)).push(r);}
    for(const [name,items] of groups){
      let law=laws.find(x=>{const n=norm(x.name);return (name.startsWith('Ley 13/1999')&&(n.includes('13/1999')||n.includes('espectaculos'))) || (name.startsWith('Decreto 155/2018')&&n.includes('155/2018'));});
      ...
      for(const r of items){
        law.sources.add(r.__source||'espectaculos_publicos.json / aforo_hosteleria_eventos.json');
        ...
        if(!old||JSON.stringify(candidate).length>JSON.stringify(old).length) map.set(k,candidate);
      }
      law.articles=[...map.values()];
      law.articles.sort((a,b)=>String(number(a)).localeCompare(String(number(b)),'es',{numeric:true}));
    }
    // Fuerza un repintado sin recargar las fuentes: el buscador existente provoca render().
