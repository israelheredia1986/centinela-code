...
(function(){
  ...
  const groups=[
    ...
    ['🏛️','ADMINISTRACIÓN Y RÉGIMEN LOCAL',['39/2015','7/1985','5/2010','policias locales','policías locales']],
    ['📚','OTRA NORMATIVA',[]],
  ...
  function addStyles(){
    ...
    const s=document.createElement('style');s.id='cc-normativa-estructura-style';s.textContent=`
      #section-normativa .normativa-list{display:block!important;padding-bottom:28px!important}
      .cc-norm-group{display:flex;align-items:center;gap:8px;margin:18px 3px 9px;padding:0 3px;color:#dff5ff;font-size:10px;font-weight:900;letter-spacing:.8px;text-transform:uppercase}
      .cc-norm-group:first-child{margin-top:4px}.cc-norm-group .cc-group-icon{font-size:19px;line-height:1;width:24px;text-align:center;filter:drop-shadow(0 0 6px rgba(49,185,255,.45))}.cc-norm-group:after{content:"";height:1px;flex:1;background:linear-gradient(90deg,rgba(49,185,255,.35),transparent)}
      .cc-norm-group span.cc-group-title{white-space:nowrap}.cc-norm-ordinanzas{margin-top:30px!important}.cc-norm-ordinanzas:before{content:"";width:3px;height:18px;border-radius:4px;background:#ffd34a;box-shadow:0 0 9px rgba(255,211,74,.55)}
      .cc-norm-ordinanzas:after{background:linear-gradient(90deg,rgba(255,211,74,.35),transparent)}
      ...
      .cc-law-card .normativa-icon{font-size:25px!important}.cc-law-card .normativa-info h3{font-size:12px!important;line-height:1.2!important}.cc-law-card .normativa-info p{font-size:8px!important}.cc-law-card .normativa-open{font-size:9px!important}.cc-law-card.cc-ordenanza{border-color:rgba(255,211,74,.28)!important}.cc-law-card.cc-ordenanza .normativa-icon{filter:drop-shadow(0 0 7px rgba(255,211,74,.3))}
      #section-normativa.cc-norm-ready .normativa-list{visibility:visible!important}.cc-norm-pending{visibility:hidden!important}
  ...
  function arrange(){
    ...
    buckets.forEach((bucket,i)=>{if(!bucket.length)return;const h=document.createElement('div');h.className='cc-norm-group'+(i===groups.length-1?' cc-norm-ordinanzas':'');h.innerHTML='<span class="cc-group-icon" aria-hidden="true">'+groups[i][0]+'</span><span class="cc-group-title">'+groups[i][1]+'</span>';frag.appendChild(h);bucket.sort((a,b)=>(a.querySelector('h3')?.textContent||'').localeCompare(b.querySelector('h3')?.textContent||'','es'));bucket.forEach(c=>frag.appendChild(c));});
    l.replaceChildren(frag);arranging=false;document.getElementById('section-normativa')?.classList.add('cc-norm-ready');
  }
  function repaint(){const s=document.getElementById('normativaSearch');if(s)s.dispatchEvent(new Event('input',{bubbles:true}));else setTimeout(arrange,80);}
  async function boot(){
    addStyles();
    const api=window.__centinelaNormativaUnificada;if(!api)return setTimeout(boot,250);
    const section=document.getElementById('section-normativa');section?.classList.add('cc-norm-pending');
    await loadOrdinances(api);
    if(!api.__centinelaEstructuraPatched){
      const original=api.reload;
      api.reload=async function(){const r=await original();await loadOrdinances(api);setTimeout(arrange,80);return r;};
      api.__centinelaEstructuraPatched=true;
    }
    const search=document.getElementById('normativaSearch');
    if(search&&!search.__ccStructureListener){search.addEventListener('input',()=>setTimeout(arrange,100));search.__ccStructureListener=true;}
    setTimeout(arrange,120);setTimeout(arrange,500);setTimeout(arrange,1000);
  }
