...
(function(){
  "use strict";
  const DATA=[
    "./data/apuntes_sppl_2026_15_150.txt.gz.b64",
    "./data/apuntes_sppl_2026_b01.txt.gz.b64"
  ];
  const norm=s=>String(s||"").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"");
  const esc=s=>String(s??"").replace(/[&<>\"]/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;"}[m]));
  let cache=null;

  async function decodeGzipB64(url){
    const r=await fetch(url+"?v=20260905",{cache:"no-store"});
    if(!r.ok) return "";
    const b64=(await r.text()).replace(/\s+/g,"");
    const bin=atob(b64);
    const bytes=new Uint8Array(bin.length);
    for(let i=0;i<bin.length;i++) bytes[i]=bin.charCodeAt(i);
    if(typeof DecompressionStream!=="function") throw new Error("El navegador no permite descomprimir el texto largo.");
  ...
  async function load(){
    ...
    const texts=await Promise.all(DATA.map(decodeGzipB64));
    const merged=texts.filter(Boolean).join("\n");
    const map=new Map();
    const re=/=====\s*P[AÁ]GINA\s+(\d+)\s*=====[\s\S]*?(?====={5,}\s*P[AÁ]GINA\s+|$)/gi;
    let m;
    while((m=re.exec(merged))){
      const raw=m[0];
      const num=Number(m[1]);
      const text=raw.replace(/^=====\s*P[AÁ]GINA\s+\d+\s*=====\s*/i,"").trim();
      if(text) map.set(num,{num,text});
    }
    const pages=[...map.values()].sort((a,b)=>a.num-b.num);
    cache={pages,text:merged};
    return cache;
  }
  ...
  function ensure(){
    ...
    const source=document.createElement("div"); source.id="ccApuntesSource"; source.className="cc-apuntes-source";
    source.innerHTML='<span>Texto largo transcrito por OCR · la pestaña Normativa queda reservada al texto legal</span>';
  ...
  function switchMode(mode){
    ...
    if(desc) desc.textContent=mode==="apuntes"?"Texto largo organizado por páginas, con búsqueda por artículo y epígrafe.":"Actuaciones y casuística para consulta rápida durante el servicio.";
  ...
  async function renderApuntes(){
    ...
    if(!list) return;
    list.innerHTML='<div class="cc-oper-loading">Cargando texto largo…</div>';
    try{
      const d=await load(); const q=norm(input?.value||"");
      const pages=q?d.pages.filter(p=>norm(p.text).includes(q)||norm("pagina "+p.num).includes(q)):d.pages;
      if(count) count.textContent=`${pages.length} páginas encontradas · ${d.pages.length} disponibles`;
      ...
      if(count) count.textContent="Error al cargar los apuntes";
    }
  }
