...
(function(){
  ...
  function style(){
    ...
    const s=document.createElement('style');s.id='ccNormativaOficialStyle';s.textContent=`
      ...
      .cc-law-actions{display:flex!important;flex-wrap:wrap!important;gap:7px!important;align-items:center!important;}
  ...
  function add(){
    ...
    const panel=document.getElementById('ccNormativaLawPanel');
    if(panel&&panel.style.display!=='none'&&!panel.querySelector('.cc-official-link')){
      const name=panel.querySelector('h3')?.textContent||'';const url=urlFor(name);if(url){const a=document.createElement('a');a.className='cc-official-link';a.href=url;a.target='_blank';a.rel='noopener noreferrer';a.textContent='↗ ABRIR ARTICULADO COMPLETO EN BOE';panel.querySelector('.cc-law-head')?.appendChild(a);}
    }
  }
