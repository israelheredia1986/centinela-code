/* CENTINELA CODE — IDENTIFICADOR DE INFRACCIONES v1.1 */
(function(){
  'use strict';
  ...
  `;
  if(!document.getElementById('cc-infra-assistant-style')){const style=document.createElement('style');style.id='cc-infra-assistant-style';style.textContent=CSS;document.head.appendChild(style)}
  ...
  function wire(){
    const btn=document.querySelector('[data-cc-infraccion-assistant="true"]') || document.querySelector('[data-cc-search-mode="infractions"]');
    if(!btn)return;
    if(btn.dataset.ccInfraWired==='true')return;
    btn.dataset.ccInfraWired='true';
    btn.removeAttribute('data-cc-search-mode');
    btn.setAttribute('data-cc-infraccion-assistant','true');
    btn.title='Identificar una posible infracción a partir de la conducta observada';
    btn.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();open();},true);
  }
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',wire);else wire();
  [300,800,1500,2500].forEach(t=>setTimeout(wire,t));
})();
