...
(function(){
  ...
  function walk(v,source,out=[],depth=0,parent=[]){
    ...
    if(article){const full=JSON.stringify(v,null,2);out.push({source,article,title,law:law||parent.join(" / "),full,text:norm(full)});}
    const next=law?[...parent,law]:parent;
    Object.values(v).forEach(c=>{if(c&&typeof c==="object")walk(c,source,out,depth+1,next)});
    return out;
  }
  async function load(){if(promise)return promise;promise=Promise.allSettled(DATA.map(async([source,url])=>{const r=await oldFetch(url,{cache:"force-cache"});if(!r.ok)throw Error("HTTP "+r.status);return walk(await r.json(),source);})).then(rs=>rs.flatMap(x=>x.status==="fulfilled"?x.value:[]));return promise;}
  ...
  window.fetch=async function(input,init){
    try{
      ...
      if(url===IA_URL&&init?.method==="POST"&&init.body){
        ...
        if(body.modo==="repository_fallback"&&q){
          const extra=await integral(q);
          if(extra){
            body.contexto=String(body.contexto||"")+extra;
            if(body.contexto.length>49000)body.contexto=body.contexto.slice(0,49000)+"\n[CONTEXTO RECORTADO POR LÍMITE DE TRANSMISIÓN]";
