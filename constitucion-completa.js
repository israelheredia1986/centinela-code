/* ============================================================
   ...
   Carga, indexación y consulta del texto consolidado.
   Fuente de datos espejo: legalize-dev/legalize-es
   Fuente jurídica de referencia: BOE
   Última actualización del BOE verificada: 20/05/2026
   ============================================================ */
(function(){
  'use strict';
  ...
  let state={data:null,query:'',onlyRelevant:false};
  ...
  function ensureStyles(){
    ...
    s.textContent=`
      ...
      .ce-btn{border:1px solid #2a709f;border-radius:11px;padding:10px 12px;background:#061a2b;color:#d9efff;font-size:9px;font-weight:900;cursor:pointer}.ce-btn.primary{background:linear-gradient(180deg,#087fe8,#0757ae);border-color:#46b7ff;color:white}.ce-btn.active{background:rgba(0,133,230,.25);border-color:#52beff;color:#fff}
  ...
  function parseMarkdown(md){
    ...
    for(let i=0;i<articleIdx.length;i++){
      ...
      let titulo='Artículo '+articleIdx[i].num;
      const heading=block.findIndex(x=>/^#{2,5}\s+/.test(x));
      if(heading===0) block.shift();
      const titleLine=block.find(x=>/^\*\*.+\*\*$/.test(x.trim()));
      if(titleLine && articles.length===0){}
      const clean=block.map(x=>x.replace(/^>\s*<small>/,'').replace(/<\/small>$/,'').replace(/^>\s*/,'').trimEnd());
      articles.push({numero:articleIdx[i].num,titulo, texto:clean.join('\n').trim()});
    }
    ...
    if(dispositionStart>=0){
      const ds=[];
      for(let i=dispositionStart;i<lines.length;i++) if(/^######\s+Disposición/.test(lines[i])) ds.push(i);
      for(let i=0;i<ds.length;i++){
        const st=ds[i]; const en=i+1<ds.length?ds[i+1]:lines.length;
  ...
  function articleMatches(a){
    const q=norm(state.query); if(!q) return true;
    return norm(`articulo ${a.numero} ${a.titulo} ${a.texto}`).includes(q);
  }

  function render(){
    const root=document.getElementById(UI_ID); if(!root||!state.data)return;
    const arts=state.data.articulos.filter(articleMatches);
    const pre=state.data.preambulo||'';
    ...
    root.querySelector('.ce-query').textContent=state.query?` · búsqueda: ${state.query}`:'';
  }
  ...
  async function loadData(){
    ...
    try{
      const res=await fetch(REMOTE_MD,{cache:'no-cache',mode:'cors'});
      if(!res.ok)throw new Error('HTTP '+res.status);
      const md=await res.text();
      const d=parseMarkdown(md);
      if(!d.articulos.length)throw new Error('El texto recibido no contiene artículos');
      state.data=d;
  ...
  function mount(){
    ...
    wrap.id=UI_ID;
    wrap.innerHTML=`
      <div class="ce-head"><div class="ce-kicker">Bloque 1 · Texto íntegro</div><h2 class="ce-title">Constitución Española</h2><p class="ce-sub">Consulta artículo por artículo del texto consolidado, con búsqueda completa y almacenamiento local para consulta posterior.</p><div class="ce-meta"><span class="ce-chip">BOE-A-1978-31229</span><span class="ce-chip">Actualización: 20/05/2026</span><span class="ce-chip">169 artículos</span><span class="ce-chip">Fuente oficial BOE</span></div></div>
      <div class="ce-toolbar"><input class="ce-search" type="search" placeholder="Buscar dentro de los 169 artículos…"/><button class="ce-btn ce-open-boe">BOE ↗</button><button class="ce-btn primary ce-clear">Mostrar todo</button></div>
      <div class="ce-index"><div class="ce-statbar"><div class="ce-stat"><strong class="ce-count">0</strong><small>Artículos visibles</small></div><div class="ce-stat"><strong>169</strong><small>Artículos</small></div><div class="ce-stat"><strong>2026</strong><small>Versión BOE</small></div><div class="ce-stat"><strong>OFFLINE</strong><small>Tras primera carga</small></div></div><div class="ce-notice">La aplicación conserva una copia local del texto cargado para facilitar su consulta posterior sin conexión. Para efectos jurídicos debe verificarse siempre la publicación oficial aplicable del BOE.</div></div>
      <div class="ce-section"><div class="ce-section-label">Articulado <span class="ce-query"></span></div><div class="ce-results"></div></div>
      <div class="ce-section ce-dispositions-wrap"><div class="ce-section-label">Disposiciones</div><div class="ce-dispositions"></div></div>
    `;
