...
const SINONIMOS_BUSQUEDA = {
  ...
  "vmp": ["mvp","patinete","patinete electrico","patinete eléctrico","vpl","vehiculo personal ligero","vehiculo de movilidad personal"],
  "mvp": ["vmp","patinete","patinete electrico","patinete eléctrico","vpl"],
  "vpl": ["vmp","mvp","vehiculo personal ligero","patinete"],
  "patinete": ["vmp","mvp","vpl","patinete electrico"],
  "registro": ["inscripcion","inscribir","registro nacional de vehiculos","rnv","revemp","matriculacion"],
  "inscripcion": ["registro","inscribir","rnv","revemp","matriculacion"],
  "trafico": ["dgt","circulacion","seguridad vial","vial"],
  ...
  "cuchillo": ["cuchillos","navaja","navajas","arma blanca","cuchilla","punzante","cortante"],
  "arma": ["armas","arma blanca","armas blancas","navaja","cuchillo","pistola","revolver","carabina","escopeta"],
  "armas": ["arma","arma blanca","navaja","cuchillo","pistola","revolver","escopeta"],
  "ruido": ["ruidos","sonido","acustico","acustica","decibelios","db","molestias","musica","fiesta"],
  "ruidos": ["ruido","sonido","acustico","decibelios","molestias","musica","fiesta"],
  "musica": ["ruido","ruidos","sonido","fiesta","local","actividad recreativa"],
  "botellon": ["bebidas alcoholicas","consumo de alcohol","via publica","botellas","fiesta"],
  "basura": ["residuos","desechos","vertidos","suciedad","escombros","residuos urbanos"],
  "escombro": ["escombros","residuos","vertido","obra","residuos de construccion"],
  "vertedero": ["vertido","residuos","basura","escombros"],
  "perro": ["perros","can","canino","animal","mascota","animal de compania"],
  "gato": ["gatos","felino","felinos","animal","colonia felina","gato comunitario"],
  "animal": ["animales","mascota","perro","gato","bienestar animal","maltrato animal"],
  "abandono": ["abandonado","abandonada","dejar solo","desamparo","animal abandonado"],
  "microchip": ["chip","identificacion","identificacion animal","transponder"],
  "chip": ["microchip","identificacion","identificacion animal"],
  "collar": ["collar electrico","collar de impulsos","collar de castigo","collar de ahogo","perro"],
  "peleas": ["peleas de animales","riñas","rinas","adiestramiento para peleas","animal"],
  "circo": ["espectaculo circense","animales","fauna silvestre","actividad prohibida"],
  "publicidad": ["reclamo publicitario","promocion","anuncio","publicidad con animales"],
  "matricula": ["matrícula","placa","placas","ilegible","identificacion del vehiculo"],
  "movil": ["móvil","telefono","teléfono","whatsapp","smartphone","dispositivo"],
  "telefono": ["teléfono","movil","móvil","whatsapp","dispositivo de telefonia"],
  "radar": ["cinemometro","cinemómetro","detector","inhibidor","velocidad"],
  "inhibidor": ["inhibidores","radar","cinemometro","cinemómetro","detector de radar"],
  "semáforo": ["semaforo","rojo","luz roja"],
  "semaforo": ["semáforo","rojo","luz roja"],
  "stop": ["ceda","ceda el paso","detenerse","señal de stop"],
  "ceda": ["stop","ceda el paso","prioridad","preferencia de paso"],
  "cinturon": ["cinturón","seguridad","retencion infantil","sistema de retencion"],
  "cinturón": ["cinturon","retencion infantil","seguridad"],
  "casco": ["proteccion","motocicleta","ciclomotor","bicicleta","seguridad"],
  "auriculares": ["cascos de audio","sonido","atencion","conduccion"],
  "itv": ["inspeccion tecnica","inspección técnica","revision","vehiculo sin itv"],
  "inspeccion": ["itv","inspección técnica","revision","vehiculo"],
  "estacionamiento": ["aparcamiento","aparcar","parada","vehiculo estacionado","parking"],
  "aparcamiento": ["estacionamiento","aparcar","parada","parking"],
  "velocidad": ["exceso de velocidad","radar","cinemometro","limite","kmh","km/h"],
  "temeraria": ["conduccion temeraria","conducir temerariamente","carreras","riesgo"],
  "negligente": ["conduccion negligente","negligencia","conducir sin diligencia"],
  "contramano": ["sentido contrario","circulacion prohibida","direccion contraria"],
  "zbe": ["zona de bajas emisiones","restricciones de circulacion","contaminacion","episodio de contaminacion"],
  "contaminacion": ["contaminación","zbe","zona de bajas emisiones","restricciones"],
  "residuos": ["basura","desechos","escombros","vertidos","restos"],
  "seguridad": ["medidas de seguridad","proteccion","vigilancia","control"]
}; 
...
function normalizarInfraccionSimple(datos) {
  ...
  return datos.map((item) => {
    ...
    return {
      ...
      palabrasClave: Array.isArray(item.palabrasClave) ? item.palabrasClave : [],
      responsables: Array.isArray(item.responsables) ? item.responsables : [],
      origenSimple: true
    };
  });
}
...
function actualizarEstadoDatos() { 
  ...
  const totalArticulosProcedimiento = obtenerLeyesProcedimientoAdministrativo()
    .reduce((total, ley) => total + ley.articulos.length, 0);
...
function obtenerArticulosNormativa() { 
  ...
  obtenerLeyesProcedimientoAdministrativo().forEach((leyItem) => {
    leyItem.articulos.forEach((articulo) => {
      registros.push({
        ley: leyItem.etiqueta || leyItem.nombre,
        leyCompleta: leyItem.nombre,
        numero: articulo.numero || articulo.articulo || "",
        titulo: articulo.titulo || articulo.nombre || "",
        texto: articulo.texto || articulo.contenido || articulo.descripcion || "",
        navTipo: "ley-procedimiento-administrativo",
        navId: leyItem.key
      });
    });
  });
...
function determinarAutoridadSancionadora(infraccion) {
  ...
  if (leyNorm.includes("39/2015") || leyNorm.includes("procedimiento administrativo")) {
    return "Órgano competente según la materia y la Administración actuante";
  }
  if (leyNorm.includes("7/1985") || leyNorm.includes("regimen local") || leyNorm.includes("régimen local")) {
    return "Órgano competente de la Entidad Local";
  }
  if (leyNorm.includes("5/2010") && leyNorm.includes("andalucia")) {
    return "Órgano competente de la Junta de Andalucía / Entidad Local";
  }
...
function obtenerContextoNormativoIA(hechos) {
  ...
  const articulosProcedimiento = obtenerLeyesProcedimientoAdministrativo()
    .flatMap((ley) => ley.articulos.map((articulo) => {
      const contenido = normalizarTexto([
        ley.nombre, ley.etiqueta, articulo.numero, articulo.titulo,
        articulo.texto, articulo.contenido, articulo.descripcion
      ].join(" "));
      let score = 0;
      tokens.forEach((token) => {
        if (contenido.includes(token)) score += 2;
        (SINONIMOS_BUSQUEDA[token] || []).forEach((sinonimo) => {
          if (contenido.includes(sinonimo)) score += 1;
        });
      });
      return { ley, articulo, score };
    }))
    .filter((x) => x.score > 0)
    .sort((a, b) => b.score - a.score)
    .slice(0, 12)
    .map((x) => ({
      ley: x.ley.nombre,
      abreviatura: x.ley.etiqueta,
      articulo: x.articulo.numero || x.articulo.articulo || "",
      titulo: x.articulo.titulo || x.articulo.nombre || "",
      texto: String(x.articulo.texto || x.articulo.contenido || x.articulo.descripcion || "").slice(0, 1200)
    }));

  return { infracciones, articulos: [...articulos, ...articulosProcedimiento] };
}
...
function renderizarNormativa() { 
  ...
  const tarjetasPrincipales = [ 
    ...
    { tipo: "procedimiento-administrativo", titulo: "Procedimiento administrativo", descripcion: `Ley 39/2015, Ley 7/1985 y Ley 5/2010 de Andalucía — ${obtenerLeyesProcedimientoAdministrativo().filter((ley) => ley.articulos.length > 0).length}/3 leyes disponibles`, etiqueta: "Administración y régimen local", icono: "📑" }, 
...
function abrirProcedimientoAdministrativo() {
  const visor = $("normativaViewer");
  const contenido = $("viewerContent");
  if (!visor || !contenido) return;
...
function abrirLeyProcedimientoAdministrativo(key) {
  const ley = obtenerLeyesProcedimientoAdministrativo().find((item) => item.key === key);
  const visor = $("normativaViewer");
  const contenido = $("viewerContent");
  if (!ley || !visor || !contenido) return;

  $("viewerTitle").textContent = ley.nombre;
  $("viewerSubtitle").textContent = `${ley.etiqueta} · ${ley.articulos.length} ${ley.articulos.length === 1 ? "artículo" : "artículos"}`;

  const articulosHTML = ley.articulos.map((articulo) => `
    <article class="law-article">
      <h4>Artículo ${escaparHTML(articulo.numero || articulo.articulo || "")} ${escaparHTML(articulo.titulo || articulo.nombre || "")}</h4>
      <p>${escaparHTML(articulo.texto || articulo.contenido || articulo.descripcion || "")}</p>
    </article>
  `).join("");

  contenido.innerHTML = `
    <div class="viewer-actions">
      <button type="button" class="normativa-open law-back-button" data-law="procedimiento-administrativo">← Volver a Procedimiento administrativo</button>
    </div>
    ${articulosHTML || `<div class="empty-state"><h3>Sin artículos cargados</h3><p>El JSON de esta ley no contiene artículos reconocibles.</p></div>`}
  `;
...
function abrirNormativa(tipo, id = "") { 
  ...
  if (tipo === "procedimiento-administrativo") return abrirProcedimientoAdministrativo(); 
  if (tipo === "ley-procedimiento-administrativo") return abrirLeyProcedimientoAdministrativo(id); 
